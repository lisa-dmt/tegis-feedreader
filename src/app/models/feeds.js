var feeds = Class.create ({
	...
	load: function() {
		this.db = new Mojo.Depot({name: "FeedListDB", version: 1, estimatedSize: 100000, replace: false},
			this.openDBSuccess.bind(this),
            function(transaction, result) {Mojo.Log.warn("Can't open feed database: ", result.message);});
	},
	...
	openDBSuccess: function() {
        this.db.get("feedList",
					this.loadFeedList.bind(this),
					function() { Mojo.Log.warn("unable to retrieve feedlist"); });
	},
	...
	save: function() {
		this.db.add("feedList", this.list,
					function() { Mojo.Log.info("feed list saved"); },
					function(transaction, result) { Mojo.Log.warn("feed list could not be saved: ", result.message); });
	},
	...
	updateFeedSuccess: function(index, transport) {
		...
		if(this.determineFeedType(index, transport)) {
			...
			// We have to cope some problems here:
			// 1. Theorhetically all stories should have an unique id, but most feeds i've seen
			//    don't follow the standard here. In case the id element cannot be found,
			//    we use the url.
			// 2. The RSS standard specifies an element "pubDate" to describe when the feed was 
			//    published, but some feeds use dc:date instead.
			// To ensure maximum compatibility we need to be tolerant about this.
				...
				case "rss":
					...
					for (i = 0; i < rssItems.length; i++) {
						try {
							...
							// Be fault tolerant and ignore a possible error here.
						}
					}
					break;
	...
	update: function() {
		...
		this.updateInProgress = this.partialUpdateInProgress;
